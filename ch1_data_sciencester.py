...
def get_bucket(tenure):
    return Observable.from_(tenure_buckets).filter(lambda tb: tb[0] <= tenure <= tb[1])
...
def words_and_counts():
    return Observable.from_(interests) \
        ...
        .to_list().map(lambda list: sorted(list, key=lambda t: t[1], reverse=True)).flat_map(
        lambda list: Observable.from_(list)) \
        .subscribe(SimplePrint())
